<template>
    <div class="card-container" :style="{'opacity': loadingStore.getActiveDownloadById(downloadId)?.loading ? 0.6 : 1}" > <!-- -->
        ...
        <div class="progress-info-container" >
            <ProgressBar style="height: 10px;" :mode="(loadingStore.getActiveDownloadById(downloadId)??{} as DownloadInProgress).progress == '' ? 'indeterminate' : 'determinate'" :value="Number((loadingStore.getActiveDownloadById(downloadId)??{} as DownloadInProgress).progress)" />
        </div>
    </div>
</template>
<script lang="ts" >
